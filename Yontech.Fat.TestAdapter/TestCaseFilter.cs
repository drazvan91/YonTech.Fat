...
{
    internal class SimpleTestCaseFilter : ITestCaseFilter
    {
        const string DISPLAY_NAME_STRING = "DisplayName";
        const string FULLY_QUALIFIED_NAME_STRING = "FullyQualifiedName";
        private readonly TestCaseFactory testCaseFactory;

        private ITestCaseFilterExpression filterExpression;

        List<string> supportedPropertyNames = new List<string>()
        {
            DISPLAY_NAME_STRING, FULLY_QUALIFIED_NAME_STRING
        };

        public SimpleTestCaseFilter(IRunContext runContext, TestCaseFactory testCaseFactory)
        {
            this.testCaseFactory = testCaseFactory;
            filterExpression = runContext.GetTestCaseFilter(supportedPropertyNames, null);
        }
    ...
    // copied from: https://github.com/xunit/visualstudio.xunit/blob/master/src/xunit.runner.visualstudio/Utility/TestCaseFilter.cs
